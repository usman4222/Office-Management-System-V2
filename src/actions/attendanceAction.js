...
export const updateUserCon = (id, attendanceData) => async (dispatch) => {
    try {
        ...
        const { data } = await axios.put(`https://soriic-b.vercel.app/api/v1/attendance/${id}`, payload, config);
...
export const getSearchAttendance = (userId, startDate, endDate) => async (dispatch) => {
    try {
        ...
        const url = `https://soriic-b.vercel.app/api/v1/searchattendance/${userId}?${queryParams}`;
...
export const getUserAttendance = (userId) => async (dispatch) => {
    try {
        ...
        if (userId) {
            const { data } = await axios.get(`https://soriic-b.vercel.app/api/v1/getuserattendance/${userId}`);
...
export const getSingleAttendanceDetails = (userId, attendanceId) => async (dispatch) => {
    try {
        ...
        const { data } = await axios.get(`https://soriic-b.vercel.app/api/v1/getsingleattendance/${userId}/${attendanceId}`);
...
export const changeStatusAction = (userId, attendanceId, userData) => async (dispatch) => {
    try {
        ...
        const { data } = await axios.put(`https://soriic-b.vercel.app/api/v1/editsingleattendance/${userId}/${attendanceId}`, userData, config);
